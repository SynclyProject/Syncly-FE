...
const DetailedNote = ({ noteId, setShowInput }: IDetailedNoteProps) => {
  ...
  // 상대방 편집 반영
  useEffect(() => {
    if (textEditorRef.current && !isComposingRef.current && !isLocalEditRef.current) {
    ...
    const connectAndSubscribe = async () => {
      try {
        ...
        const handleWebSocketError = (error: any) => {
          ...
          if (error?.payload?.code === "Note409_1" || error?.payload?.code === "Note409_2") {
            console.log("🔄 OT 충돌 감지");

            if (error.payload.content !== undefined && error.payload.revision !== undefined) {
              isResyncingRef.current = true;
              ...
              try {
                ...
                setError("편집 충돌이 발생했습니다. 최신 버전으로 복구되었습니다.");
              } finally {
                isResyncingRef.current = false;
              }
            }
        ...
        const handleEdit = (payload: TEditPayload) => {
          ...
          if (payload.operation.workspaceMemberId === memberId) {
            ...
            console.log("🔄 자신의 편집 확인됨:", {
              receivedRevision,
              removed: currentState.pendingOperations.length - newPending.length,
            });
        ...
        const errorMsg =
          err instanceof Error ? err.message : "실시간 편집 연결에 실패했습니다.";
        setError(errorMsg);
        setLoading(false);
      }
    };
  ...
  }, [noteId, memberId, setCurrentNote, handleRemoteEdit, setError, setLoading]);
    ...
    try {
      ...
      Promise.resolve().then(() => {
        ...
        if (pendingRemoteEditsRef.current.length > 0) {
          console.log(`📤 ${pendingRemoteEditsRef.current.length}개 remote edit 처리`);
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      <div className="h-[56px] flex items-center gap-5 bg-white rounded-t-[8px] p-3 border border-[#E0E0E0]">
        ...
        <Button colorType="sub" iconName="Pen" onClick={() => setShowInput(true)} />
      </div>
