  ...
  useEffect(() => {
    ...
    const fetchNotes = async () => {
      try {
        ...
        console.log(`📡 노트 목록 조회 시작: workspaceId=${workspaceId}, page=${currentPage}`);
        ...
        if (!Array.isArray(response.notes)) {
          ...
          throw new Error(`response.notes는 배열이어야 하는데 ${typeof response.notes}입니다`);
        }

        setNotes(response.notes);
        setTotalPages(response.totalPages || 0);

        console.log(`✨ 노트 ${response.notes.length}개 로드됨, 총 ${response.totalPages || 0}페이지`);
      } catch (err) {
        console.error("❌ 노트 목록 조회 실패:", err);
        const errorMsg = (err as Error).message || "노트 목록을 조회할 수 없습니다";
        setError(errorMsg);
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-[56px] bg-white flex items-center gap-[63px]">
        <p className="flex-1 text-[16px] font-semibold pl-[20px]">제목</p>
        <p className="text-[16px] font-semibold">수정일</p>
        <p className="text-[16px] font-semibold pr-[80px]">작성자</p>
      </div>
            ...
            <>
              {sortedNotes.map((note: TNotes) => (
                <Note
                  key={note.id}
                  title={note.title}
                  date={new Date(note.lastModifiedAt).toLocaleDateString("ko-KR")}
                  noteId={note.id}
