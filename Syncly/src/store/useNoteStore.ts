...
export interface NoteStoreActions {
  ...
  // Yjs 초기화 및 정리
  initializeYjs: (noteId: number, ydocBinary: string | null, currentWorkspaceMemberId?: number) => void;
  cleanupYjs: () => void;
  reset: () => void;
...
export const useNoteStore = create<NoteStore>((set, get) => ({
  ...
  // ===== Yjs 초기화 및 정리 =====
  initializeYjs: (noteId: number, ydocBinary: string | null, currentWorkspaceMemberId?: number) => {
    const state = get();
    ...
    if (ydocBinary) {
      try {
        ...
        console.log("🔍 [Step 1] Base64 디코딩 시작:", {
          ...
          last20chars: ydocBinary.substring(Math.max(0, ydocBinary.length - 20))
        });

        const binaryString = atob(ydocBinary);
        console.log("🔍 [Step 1] Base64 디코딩 완료:", {
          decodedStringLength: binaryString.length,
          firstCharCodes: Array.from(binaryString.slice(0, 10)).map(c => c.charCodeAt(0)),
          lastCharCodes: Array.from(binaryString.slice(-10)).map(c => c.charCodeAt(0))
        });
        ...
        console.log("🔍 [Step 2] Uint8Array 변환 완료:", {
          ...
          firstBytes: Array.from(binaryUpdate.slice(0, 10)).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', '),
          lastBytes: Array.from(binaryUpdate.slice(-10)).map(b => '0x' + b.toString(16).padStart(2, '0')).join(', ')
        });
    ...
    const ydocObserver = (update: Uint8Array, origin: any) => {
      console.log("🔍 Y.Doc Update 이벤트 발생:", { updateSize: update.length, origin, originType: typeof origin });
      ...
      if (origin !== null) {
        ...
        try {
          ...
          const base64Update = btoa(String.fromCharCode.apply(null, Array.from(fullState)));
          console.log("🔄 Base64 Update 생성 (전체 상태):", { size: base64Update.length });

          // 서버로 전송하도록 이벤트 발생 (DetailedNote에서 처리)
          console.log("🎯 CustomEvent 'yjsUpdateReady' 발생");
          window.dispatchEvent(
            new CustomEvent("yjsUpdateReady", { detail: { base64Update, noteId } })
          );
        } catch (error) {
    ...
    const awarenessObserver = ({ added, updated }: any) => {
